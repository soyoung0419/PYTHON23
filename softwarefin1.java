...
public class CampusPlannerConsole {
   ...
   static class TaskDao implements Crud<Task> {
    ...
    // author: jaeeun0109, expected: 할일 CRUD + 임박조회 , developed: 추가/수정/삭제/목록, 임박 필터, 검색(제목/키워드/기간), 카테고리 필터, 아카이브 이동, ID 재정렬, 저장 반영
    static class ScheduleDao implements Crud<Schedule> {
        public final LocalStore store;
        private final LocalDatabaseManager db;

        ScheduleDao(LocalStore store, LocalDatabaseManager db) {
            if (store == null) {
                this.store = null;
            } else {
                this.store = store;
        ...
        public static final Map<String, List<CourseInfo>> COURSE_MAP = new HashMap<>();
        ...
        static class CourseInfo {
            ...
            CourseInfo(String professor, String room, String start, String end, DayOfWeek day) {
        ...
        static {
            COURSE_MAP.put("자료구조", Arrays.asList(
                new CourseInfo("김철수", "415호", "09:00", "13:00", DayOfWeek.MONDAY),
                new CourseInfo("박영희", "416호", "14:00", "18:00", DayOfWeek.WEDNESDAY)
            ));
            COURSE_MAP.put("운영체제", Arrays.asList(
                new CourseInfo("이영희", "210호", "10:00", "12:00", DayOfWeek.TUESDAY)
            ));
            COURSE_MAP.put("컴퓨터구조", Arrays.asList(
                new CourseInfo("박민수", "305호", "10:00", "12:00", DayOfWeek.THURSDAY)
            ));
            COURSE_MAP.put("데이터베이스", Arrays.asList(
                new CourseInfo("이영희", "220호", "14:00", "18:00", DayOfWeek.FRIDAY)
            ));
            COURSE_MAP.put("인공지능", Arrays.asList(
                new CourseInfo("김철수", "310호", "14:00", "18:00", DayOfWeek.WEDNESDAY)
            ));
        }

        List<CourseInfo> suggestCourses(String courseName) {
        ...
        Schedule addNew(String course, String professor, String room, DayOfWeek dow,
                        LocalTime start, LocalTime end, String memo) {
            Set<Long> used = new HashSet<>();
            for (int i = 0; i < store.schedules.size(); i++) {
                used.add(store.schedules.get(i).id);
            }
            long newId = 1;
            while (used.contains(newId)) {
                newId = newId + 1;
            }
            Schedule s = new Schedule(newId, course, professor, room, dow, start, end, memo);
            store.schedules.add(s);
            if (store.scheduleSeq <= newId) {
                store.scheduleSeq = newId + 1;
            }
            if (db != null) {
                db.save(store);
            }
        ...
        boolean isDuplicate(String course, String professor) {
            for (int i = 0; i < store.schedules.size(); i++) {
                Schedule s = store.schedules.get(i);
                if (s.courseName.equalsIgnoreCase(course)) {
                    if (s.professor.equalsIgnoreCase(professor)) {
                        return true;
                    }
                }
            }
        ...
        public Optional<Schedule> get(long id) {
            for (int i = 0; i < store.schedules.size(); i++) {
                Schedule x = store.schedules.get(i);
                if (x.id == id) {
                    return Optional.of(x);
                }
            }
        ...
        public List<Schedule> getAll() {
            ...
            for (int i = 0; i < store.schedules.size(); i++) {
                list.add(store.schedules.get(i));
            }
            return list;
        }
        ...
        public void update(Schedule s) {
            Optional<Schedule> opt = get(s.id);
            if (opt.isPresent()) {
                Schedule old = opt.get();
                old.courseName = s.courseName;
                old.professor = s.professor;
                old.room = s.room;
                old.dow = s.dow;
                old.start = s.start;
                old.end = s.end;
                ...
                System.out.println("업데이트 실패: 항목 없음");
            }
        }

        @Override
        public void delete(long id) {
            Optional<Schedule> opt = get(id);
            if (opt.isPresent()) {
                Schedule s = opt.get();
                store.schedules.remove(s);
                resequenceIds();
                if (db != null) {
                    db.save(store);
                }
            } else {
                System.out.println("삭제 실패: 해당 ID 없음");
            }
        }

        private void resequenceIds() {
            List<Schedule> old = new ArrayList<>();
            for (int i = 0; i < store.schedules.size(); i++) {
                old.add(store.schedules.get(i));
            }

            List<Schedule> fresh = new ArrayList<>();
            long nid = 1;
            for (int i = 0; i < old.size(); i++) {
                Schedule s = old.get(i);
                Schedule ns = new Schedule(nid, s.courseName, s.professor, s.room, s.dow, s.start, s.end, s.memo);
                fresh.add(ns);
                nid = nid + 1;
            }
            ...
            for (int i = 0; i < fresh.size(); i++) {
                store.schedules.add(fresh.get(i));
            }

            store.scheduleSeq = nid;
        }

        List<Schedule> byDay(DayOfWeek dow) {
            ...
            for (int i = 0; i < result.size(); i++) {
                for (int j = i + 1; j < result.size(); j++) {
                    Schedule a = result.get(i);
                    Schedule b = result.get(j);
        ...
        boolean hasOverlap(Schedule candidate) {
            for (int i = 0; i < store.schedules.size(); i++) {
                Schedule s = store.schedules.get(i);
                boolean overlap = s.overlaps(candidate);
                if (overlap == true) {
                    return true;
                }
            }
        ...
        boolean existsCourse(String courseName) {
            for (int i = 0; i < store.schedules.size(); i++) {
                Schedule s = store.schedules.get(i);
                if (s.courseName.equalsIgnoreCase(courseName)) {
                    return true;
                }
            }
            return false;
    ...
    // author: hxeonsu, expected: 성적 CRUD + 과목검증 , developed: byCourse/평균/요약 제공 및 시간표 과목 존재 검증
    static class GradeDao implements Crud<GradeEntry> {
        private final LocalStore store; private final LocalDatabaseManager db;
        GradeDao(LocalStore store, LocalDatabaseManager db){ this.store = store; this.db = db; }
        GradeEntry addNew(String courseName, double score){
            long id = store.gradeSeq++;
            GradeEntry g = new GradeEntry(id, courseName, score);
            store.grades.add(g); db.save(store); return g;
        }
        @Override public GradeEntry add(GradeEntry g){ store.grades.add(g); db.save(store); return g; }
        @Override public Optional<GradeEntry> get(long id){ return store.grades.stream().filter(x->x.id==id).findFirst(); }
        @Override public List<GradeEntry> getAll(){ return new ArrayList<>(store.grades); }
        @Override public void update(GradeEntry g){
            get(g.id).ifPresent(old -> {
                old.courseName = g.courseName;
                old.score = g.score;
                db.save(store);
            });
        }
        @Override public void delete(long id){ get(id).ifPresent(x->{ store.grades.remove(x); db.save(store); }); }
        Optional<GradeEntry> byCourse(String course){
            return store.grades.stream().filter(x->x.courseName.equalsIgnoreCase(course)).findFirst();
        }
    }
    ...
    // author: jaeeun0109, expected: 과제 CRUD + 임박조회 , developed: add/update/delete/list + 과목/키워드 검색 + 마감순 정렬
    static class AssignmentDao implements Crud<Assignment> {
        private final LocalStore store;
        private final LocalDatabaseManager db;

        AssignmentDao(LocalStore store, LocalDatabaseManager db) {
            this.store = store;
            this.db = db;
        }

        Assignment addNew(String courseName, String title, LocalDateTime dueAt,
                          String submitMethod, String memo) {
            long id = store.assignmentSeq;
            store.assignmentSeq = store.assignmentSeq + 1;
            Assignment a = new Assignment(id, courseName, title, dueAt, false, submitMethod, memo);
        ...
        @Override
        public Assignment add(Assignment a) {
            store.assignments.add(a);
            if (db != null) {
                db.save(store);
            }
        ...
        public Optional<Assignment> get(long id) {
            for (int i = 0; i < store.assignments.size(); i++) {
                Assignment a = store.assignments.get(i);
                if (a.id == id) {
        ...
        public List<Assignment> getAll() {
            ...
            for (int i = 0; i < store.assignments.size(); i++) {
                out.add(store.assignments.get(i));
            }
            return out;
        }
        ...
        public void update(Assignment a) {
            Optional<Assignment> opt = get(a.id);
            if (opt.isPresent()) {
                Assignment old = opt.get();
                old.courseName = a.courseName;
                old.title = a.title;
                old.dueAt = a.dueAt;
                old.submitted = a.submitted;
                old.submitMethod = a.submitMethod;
                old.memo = a.memo;
                if (db != null) {
                    db.save(store);
                }
            } else {
                System.out.println("업데이트할 과제 없음");
            }
        }

        @Override
        public void delete(long id) {
            Optional<Assignment> opt = get(id);
            if (opt.isPresent()) {
                ...
                System.out.println("삭제 대상 과제 없음");
            }
        }

        List<Assignment> dueWithin(Duration d) {
            LocalDateTime now = LocalDateTime.now();
            List<Assignment> out = new ArrayList<>();
            for (int i = 0; i < store.assignments.size(); i++) {
                Assignment a = store.assignments.get(i);
                if (a.dueAt != null) {
                    Duration diff = Duration.between(now, a.dueAt);
                    if (diff.toMillis() >= 0 && diff.compareTo(d) <= 0) {
                        out.add(a);
                    }
                }
            }
            // 마감 임박 목록 마감 시각 기준 정렬
            for (int i = 0; i < out.size(); i++) {
                for (int j = i + 1; j < out.size(); j++) {
                    Assignment x = out.get(i);
                    Assignment y = out.get(j);
                    LocalDateTime xd = (x.dueAt == null ? LocalDateTime.MAX : x.dueAt);
                    LocalDateTime yd = (y.dueAt == null ? LocalDateTime.MAX : y.dueAt);
                    if (xd.isAfter(yd)) {
                        out.set(i, y);
                        out.set(j, x);
                    }
        ...
        List<Assignment> findByCourse(String courseName) {
            List<Assignment> out = new ArrayList<>();
            for (int i = 0; i < store.assignments.size(); i++) {
                Assignment a = store.assignments.get(i);
                if (a.courseName != null && a.courseName.equalsIgnoreCase(courseName)) {
                    out.add(a);
                }
            }
            return out;
        ...
        List<Assignment> searchByKeyword(String key) {
            List<Assignment> out = new ArrayList<>();
            ...
            for (int i = 0; i < store.assignments.size(); i++) {
                Assignment a = store.assignments.get(i);
                boolean match = false;
                if (a.title != null && a.title.contains(key)) match = true;
                if (!match && a.courseName != null && a.courseName.contains(key)) match = true;
                if (!match && a.memo != null && a.memo.contains(key)) match = true;
                if (match) {
                    out.add(a);
                }
            }
        ...
        Optional<Assignment> nextFromNow() {
            LocalDateTime now = LocalDateTime.now();
    ...
    // ====== Services / ViewModels ======
    // author: jaeeun0109, expected: 할일 알림 스케줄링 , developed: ScheduledExecutorService로 마감 시각 콘솔 알림 예약
    static class NotificationHelper { 
        private final ScheduledExecutorService sched = Executors.newScheduledThreadPool(1);
        private final Map<Long, ScheduledFuture<?>> booked = new ConcurrentHashMap<>();
        void scheduleTaskAlarm(Task t){
            if (t.dueAt==null) return;
            long delayMs = Math.max(0, Duration.between(LocalDateTime.now(), t.dueAt).toMillis());
            cancelTaskAlarm(t.id);
            ScheduledFuture<?> f = sched.schedule(() -> {
                System.out.println("[알림] 할 일 마감: " + t);
            }, delayMs, TimeUnit.MILLISECONDS);
            booked.put(t.id, f);
        }
        void cancelTaskAlarm(long taskId){ Optional.ofNullable(booked.remove(taskId)).ifPresent(f->f.cancel(false)); }
        void shutdown(){ sched.shutdownNow(); }
    }

    // author: jaeeun0109, expected: 할일 비즈니스 로직(알림/정렬/토글) , developed: CRUD 위임, 완료 처리, 수정 기능, 정렬된 목록 제공, 검색·필터·아카이브 조회, 알림 스케줄링 및 재예약

    static class TaskViewModel {
        private final TaskDao dao;
        private final NotificationHelper notifier;

        TaskViewModel(TaskDao dao, NotificationHelper notifier) {
            if (dao == null) {
                this.dao = null;
            } else {
                this.dao = dao;
        ...
        Task add(String title, LocalDateTime dueAt, String memo) {
            Task t = dao.addNew(title, dueAt, memo);
            if (dueAt != null) {
                if (!t.completed) {
                    if (dueAt.isAfter(LocalDateTime.now())) {
                        if (notifier != null) {
                            notifier.scheduleTaskAlarm(t);
                        }
                    }
                }
            }
        ...
        void toggleDone(long id, boolean done) {
            ...
            if (opt.isPresent()) {
                ...
                if (done == true) {
                    if (notifier != null) {
                        notifier.cancelTaskAlarm(id);
                    }
                    dao.delete(id);
                    System.out.println("완료된 할일이 자동 삭제되었습니다.");
                } else {
                    dao.update(t);
                    if (t.dueAt != null) {
                        if (t.dueAt.isAfter(LocalDateTime.now())) {
                            if (notifier != null) {
                                notifier.scheduleTaskAlarm(t);
                            }
                        }
                ...
                System.out.println("해당 ID의 할일을 찾을 수 없습니다.");
            }
        }
        ...
        void delete(long id) {
            ...
            System.out.println("해당 할일이 아카이브로 이동되었습니다.");
        }

        List<Task> list() {
            List<Task> l = dao.getAll();
            for (int i = 0; i < l.size(); i++) {
                for (int j = i + 1; j < l.size(); j++) {
                    Task a = l.get(i);
                    Task b = l.get(j);
                    LocalDateTime adt = (a.dueAt == null ? LocalDateTime.MAX : a.dueAt);
                    LocalDateTime bdt = (b.dueAt == null ? LocalDateTime.MAX : b.dueAt);
                    if (adt.isAfter(bdt)) {
                        l.set(i, b);
                        l.set(j, a);
                    }
        ...
        void update(long id, String title, LocalDateTime dueAt,
            String memo, String category, int priority) {
            ...
            if (opt.isPresent()) {
                ...
                t.update(title, dueAt, memo, category, priority);

                dao.update(t);

                if (notifier != null) {
                    notifier.cancelTaskAlarm(id);
                    if (!t.completed && t.dueAt != null && t.dueAt.isAfter(LocalDateTime.now())) {
                        notifier.scheduleTaskAlarm(t);
                    }
                }
                ...
                System.out.println("해당 ID의 할일이 없습니다.");
            }
        }

        List<Task> dueSoon(Duration d) {
            return dao.dueWithin(d);
        }

        List<Task> searchTitle(String key) { return dao.searchByTitle(key); }

        List<Task> searchKeyword(String key) { return dao.searchByKeyword(key); }

        List<Task> searchPeriod(LocalDate from, LocalDate to) { return dao.searchByPeriod(from, to); }

        List<Task> filterCategory(String cat) { return dao.filterByCategory(cat); }

        List<Task> archiveList() { return dao.getArchive(); }
        
        void rebookAllAlarms() {
    ...
    // author: jaeeun0109, expected: 시간표 추가 시 겹침 방지·정렬 , developed: overlaps 검사 + weekly/nextFromNow 제공
    static class ScheduleViewModel {
        final ScheduleDao dao;

        ScheduleViewModel(ScheduleDao dao) {
            if (dao == null) {
                System.out.println("경고: ScheduleDao가 null입니다.");
                this.dao = null;
            } else {
                this.dao = dao;
            }
        ...
        Optional<Schedule> add(String course, String professor, String room,
                            DayOfWeek dow, LocalTime start, LocalTime end, String memo) {
            if (start.isBefore(end) == false) {
                throw new IllegalArgumentException("시작시간은 종료시간보다 이전이어야 합니다.");
            }

            Schedule candidate = new Schedule(-1, course, professor, room, dow, start, end, memo);

            boolean overlap = false;
            if (dao != null) {
                if (dao.hasOverlap(candidate)) {
                    overlap = true;
                }
            }
            ...
            if (overlap == true) {
                ...
                Schedule s = dao.addNew(course, professor, room, dow, start, end, memo);
        ...
        List<Schedule> weekly() {
            ...
            for (int i = 0; i < list.size(); i++) {
                for (int j = i + 1; j < list.size(); j++) {
                    Schedule a = list.get(i);
                    Schedule b = list.get(j);
                    int dayDiff = a.dow.getValue() - b.dow.getValue();
                    if (dayDiff > 0 || (dayDiff == 0 && a.start.isAfter(b.start))) {
                        list.set(i, b);
                        list.set(j, a);
                    }
                }
        ...
        Optional<Schedule> nextFromNow() {
            LocalDateTime now = LocalDateTime.now();
            List<Schedule> all = dao.getAll();
            if (all.isEmpty()) {
                return Optional.empty();
            }
            Schedule earliest = all.get(0);
            LocalDateTime earliestTime = earliest.nextOccurrenceFrom(now);
            for (int i = 1; i < all.size(); i++) {
                Schedule s = all.get(i);
                LocalDateTime t = s.nextOccurrenceFrom(now);
                if (t.isBefore(earliestTime)) {
                    earliest = s;
                    earliestTime = t;
                }
            }
        ...
        void delete(long id) {
            if (dao != null) {
                ...
                System.out.println("삭제 실패: dao가 null입니다.");
            }
        }

        List<Schedule> byDay(DayOfWeek d) {
            List<Schedule> result = new ArrayList<>();
            List<Schedule> all = dao.getAll();
            for (int i = 0; i < all.size(); i++) {
                Schedule s = all.get(i);
        ...
        boolean existsCourse(String name) {
            boolean found = false;
            List<Schedule> all = dao.getAll();
            for (int i = 0; i < all.size(); i++) {
                Schedule s = all.get(i);
                if (s.courseName.equalsIgnoreCase(name)) {
                    found = true;
                    break;
                }
            }
    ...
    // author: hxeonsu, expected: 성적 비즈니스 로직(GPA/요약/검증) , developed: 평균/등급/포인트 계산 + 시간표 과목 유효성 확인
    static class GradeViewModel {
        private final GradeDao dao;
        private final ScheduleViewModel scheduleVM;

        GradeViewModel(GradeDao dao, ScheduleViewModel scheduleVM){
            this.dao = dao; this.scheduleVM = scheduleVM;
        }

        GradeEntry add(String courseName, double score){
            validateCourseExists(courseName);
            validateScore(score);
            Optional<GradeEntry> exist = dao.byCourse(courseName);
            if (exist.isPresent()) throw new IllegalArgumentException("이미 등록된 과목입니다. u.수정을 사용하세요.");
            return dao.addNew(courseName, score);
        }

        void update(long id, String newCourseName, Double newScore){
            dao.get(id).ifPresentOrElse(g -> {
                if (newCourseName != null && !newCourseName.isBlank()){
                    validateCourseExists(newCourseName);
                    g.courseName = newCourseName;
                }
                if (newScore != null){
                ...
                dao.update(g);
            }, ()-> { throw new IllegalArgumentException("해당 ID의 성적이 없습니다."); });
        }

        void delete(long id){ dao.delete(id); }
        List<GradeEntry> list(){
            List<GradeEntry> l = dao.getAll();
            l.sort(Comparator.comparing(ge->ge.courseName.toLowerCase(Locale.ROOT)));
            return l;
        }
        ...
        double averageScore(){
            ...
            double sum = 0.0; for (GradeEntry g : l) sum += g.score; return sum / l.size();
        }

        String gpaLetter(){ return toLetter(averageScore()); }

        static String toLetter(double score){
        ...
        static double toGpaPoint(String letter){ // 4.5 만점 스케일
            switch (letter){
                case "A+": return 4.5;
                case "A0": return 4.0;
                case "B+": return 3.5;
        ...
        Map<String, Object> summary(){
            ...
            String letter = toLetter(avg);
            double point = toGpaPoint(letter);
            GradeEntry max = Collections.max(l, Comparator.comparingDouble(g->g.score));
            GradeEntry min = Collections.min(l, Comparator.comparingDouble(g->g.score));
            m.put("avg", round1(avg));
            m.put("gpaLetter", letter);
            m.put("gpaPoint", point);
            m.put("max", String.format(Locale.ROOT, "%s(%.2f)", max.courseName, max.score));
            m.put("min", String.format(Locale.ROOT, "%s(%.2f)", min.courseName, min.score));
            return m;
        }

        private static double round1(double v){ return Math.round(v*10.0)/10.0; }
        private void validateCourseExists(String courseName){
            if (!scheduleVM.existsCourse(courseName))
                throw new IllegalArgumentException("시간표에 없는 과목입니다. 시간표에 먼저 추가하세요.");
        }
    ...
    // author: jaeeun0109, expected: 과제 비즈니스 로직(추가/목록/제출/삭제/임박조회) , developed: CRUD 위임, 제출 시 자동 삭제, 마감순 정렬, 검색·과목 필터
    static class AssignmentViewModel {
        private final AssignmentDao dao;

        AssignmentViewModel(AssignmentDao dao) {
            this.dao = dao;
        }

        Assignment add(String courseName, String title, LocalDateTime dueAt,
                       String submitMethod, String memo) {
            return dao.addNew(courseName, title, dueAt, submitMethod, memo);
        }
        ...
        List<Assignment> list() {
            ...
            for (int i = 0; i < list.size(); i++) {
                for (int j = i + 1; j < list.size(); j++) {
                    ...
                    Assignment b = list.get(j);
                    LocalDateTime ad = (a.dueAt == null ? LocalDateTime.MAX : a.dueAt);
                    LocalDateTime bd = (b.dueAt == null ? LocalDateTime.MAX : b.dueAt);
                    if (ad.isAfter(bd)) {
                        list.set(i, b);
                        list.set(j, a);
                    }
        ...
        void submit(long id) {
            Optional<Assignment> opt = dao.get(id);
            if (opt.isPresent()) {
                Assignment a = opt.get();
                a.submitted = true;
                dao.update(a);
                dao.delete(id);
                System.out.println("제출된 과제가 자동 삭제되었습니다.");
            } else {
                System.out.println("해당 ID의 과제를 찾을 수 없습니다.");
            }
        }
        ...
        void update(long id, String newCourseName, String newTitle,
                    LocalDateTime newDueAt, String newMethod, String newMemo) {
            Optional<Assignment> opt = dao.get(id);
            if (opt.isPresent()) {
                Assignment a = opt.get();
                if (newCourseName != null) a.courseName = newCourseName;
                if (newTitle != null) a.title = newTitle;
                if (newDueAt != null) a.dueAt = newDueAt;
                if (newMethod != null) a.submitMethod = newMethod;
                if (newMemo != null) a.memo = newMemo;
                dao.update(a);
                System.out.println("수정 완료됨.");
            } else {
                System.out.println("해당 ID의 과제가 없습니다.");
            }
        }

        List<Assignment> dueSoon(Duration d) {
            return dao.dueWithin(d);
        }

        List<Assignment> byCourse(String courseName) {
            return dao.findByCourse(courseName);
        }

        List<Assignment> searchKeyword(String key) {
            return dao.searchByKeyword(key);
        }

        Optional<Assignment> nextDeadline() {
            return dao.nextFromNow();
        }
    }
    ...
    // === Handlers ===
    // author: jaeeun0109, expected: 할일 인터랙션(추가/목록/완료/삭제/임박확인 등) , developed: TaskViewModel 호출 로직
    static void handleTasks(Scanner sc, TaskViewModel vm) {
        ...
        while (running == true) {
            ...
            System.out.println(" l: 목록 보기");
            System.out.println(" t: 완료 설정");
            System.out.println(" d: 삭제");
            System.out.println(" s: 임박 확인");
            System.out.println(" u: 수정");
            System.out.println(" c: 카테고리 필터");
            System.out.println(" k: 키워드 검색");
            System.out.println(" f: 기간 검색");
            System.out.println(" r: 아카이브 보기");
            ...
            if (c.equals("b")) {
                ...
                if (c.equals("a")) {
                    ...
                    System.out.print("마감(yyyy-MM-dd HH:mm, 비우면 없음): ");
                    String ds = sc.nextLine().trim();
                    LocalDateTime due;
                    if (ds.equals("")) {
                        due = null;
                    } else {
                        due = DateTimeUtils.parseDateTime(ds);
                    }
                    
                    System.out.print("메모(선택): ");
                    String memo = sc.nextLine();
                    
                    Task t = vm.add(title, due, memo);
                    System.out.println("추가됨: " + t);
                    
                } else if (c.equals("l")) {
                    List<Task> list = vm.list();
                    for (int i = 0; i < list.size(); i++) {
                    ...
                    System.out.print("완료 true/false: ");
                    boolean done;
                    ...
                    System.out.print("임박 임계(일, 1~7 사이 입력): ");
                    String input = sc.nextLine();
                    long days = Long.parseLong(input);
                    if (days < 1) {
                        System.out.println("1일 이상 입력해야 합니다.");
                    } else if (days > 7) {
                        System.out.println("7일 이하만 입력 가능합니다.");
                    } else {
                        List<Task> soon = vm.dueSoon(Duration.ofDays(days));
                        for (int i = 0; i < soon.size(); i++) {
                            Task t = soon.get(i);
                            System.out.println("임박(" + days + "일 이내): " + t);
                    ...
                    System.out.print("새 제목(비우면 기존 유지): ");
                    String title = sc.nextLine();
                    if (title.trim().equals("")) title = null;
    
                    System.out.print("새 마감(yyyy-MM-dd HH:mm, 비우면 기존 유지): ");
                    String ds = sc.nextLine().trim();
                    LocalDateTime due;
                    if (ds.equals("")) due = null;
                    else due = DateTimeUtils.parseDateTime(ds);
    
                    System.out.print("새 메모(비우면 기존 유지): ");
                    String memo = sc.nextLine();
                    if (memo.trim().equals("")) memo = null;
    
                    System.out.print("카테고리(비우면 기존 유지): ");
                    String category = sc.nextLine();
                    if (category.trim().equals("")) category = null;
    
                    System.out.print("우선순위 숫자 입력(비우면 기존 유지): ");
                    String p = sc.nextLine();
                    int priority;
                    if (p.trim().equals("")) priority = -99999; 
                    else priority = Integer.parseInt(p);
    
                    vm.update(id, title, due, memo, category, priority);
                    ...
                    List<Task> list = vm.filterCategory(cat);
                    ...
                    List<Task> list = vm.searchKeyword(key);
                    for (Task t : list) System.out.println(t);
                }
    
                else if (c.equals("f")) {
                    System.out.print("기간 시작일(yyyy-MM-dd): ");
                    LocalDate from = DateTimeUtils.parseDate(sc.nextLine().trim());
    
                    System.out.print("기간 종료일(yyyy-MM-dd): ");
                    LocalDate to = DateTimeUtils.parseDate(sc.nextLine().trim());
    
                    List<Task> list = vm.searchPeriod(from, to);
                    for (Task t : list) System.out.println(t);
                }
    
                else if (c.equals("r")) {
                    List<Task> arch = vm.archiveList();
                    for (Task t : arch) {
                        System.out.println("[아카이브] " + t);
    ...
    // author: jaeeun0109, expected: 시간표 인터랙션(추가/조회/다음수업/삭제/요일별) , developed: ScheduleViewModel 호출 로직
    static void handleSchedule(Scanner sc, ScheduleViewModel vm) {
        boolean loop = true;
        ...
        while (loop) {
            ...
            System.out.println("========================================");
            System.out.println(" a  : 추가");
            System.out.println(" l  : 주간 목록");
            System.out.println(" n  : 다음 수업");
            System.out.println(" d  : 삭제");
            System.out.println(" day: 요일별 조회");
            System.out.println(" b  : 뒤로가기");
            System.out.println("----------------------------------------");
            ...
            if (c.equals("b")) {
                ...
                Map<String, List<ScheduleDao.CourseInfo>> cmap = ScheduleDao.COURSE_MAP;
                List<ScheduleDao.CourseInfo> list;
                if (cmap.containsKey(cn)) {
                    list = cmap.get(cn);
                } else {
                    list = new ArrayList<>();
                }
                ...
                if (list.size() > 0) {
                    System.out.println("자동 등록 가능한 수업 목록:");
                    for (int i = 0; i < list.size(); i++) {
                        ScheduleDao.CourseInfo info = list.get(i);
                        System.out.println((i + 1) + ". " + info.professor + " 교수 | " + info.room + " | " + info.start + "~" + info.end + " | " + info.day);
                    }
                    System.out.print("번호 선택 (직접입력은 0): ");
                    String selStr = sc.nextLine().trim();
                    int sel = Integer.parseInt(selStr);

                    if (sel >= 1 && sel <= list.size()) {
                        ScheduleDao.CourseInfo cinfo = list.get(sel - 1);
                        System.out.print("메모(선택): ");
                        String memo = sc.nextLine();
                        Optional<Schedule> added = vm.add(
                            cn,
                            cinfo.professor,
                            cinfo.room,
                            cinfo.day,
                            LocalTime.parse(cinfo.start),
                            LocalTime.parse(cinfo.end),
                            memo
                        );
                        if (added.isPresent()) {
                            System.out.println("자동 등록 완료: " + added.get());
                        } else {
                            System.out.println("겹치는 시간으로 등록 실패");
                        }
                        continue;
                    }
                }

                System.out.println("자동 목록 없음 — 수동 입력으로 전환");
                System.out.print("교수명: ");
                String prof = sc.nextLine();
                System.out.print("강의실: ");
                String room = sc.nextLine();
                System.out.print("요일(MON..SUN 또는 월~일): ");
                String dayStr = sc.nextLine();
                DayOfWeek dow = DateTimeUtils.parseKoreanDow(dayStr);
                System.out.print("시작(HH:mm): ");
                String stStr = sc.nextLine();
                LocalTime st = DateTimeUtils.parseTime(stStr);
                System.out.print("종료(HH:mm): ");
                String etStr = sc.nextLine();
                LocalTime et = DateTimeUtils.parseTime(etStr);
                System.out.print("메모(선택): ");
                String memo = sc.nextLine();
                Optional<Schedule> added = vm.add(cn, prof, room, dow, st, et, memo);
                if (added.isPresent()) {
                    System.out.println("추가됨: " + added.get());
                } else {
                    System.out.println("겹치는 시간 존재하여 추가 실패");
                }
            } else if (c.equals("l")) {
                List<Schedule> all = vm.weekly();
                for (int i = 0; i < all.size(); i++) {
                    System.out.println(all.get(i));
                }
            } else if (c.equals("n")) {
                Optional<Schedule> next = vm.nextFromNow();
                if (next.isPresent()) {
                    Schedule s = next.get();
                    LocalDateTime time = s.nextOccurrenceFrom(LocalDateTime.now());
                    System.out.println("다음 수업: " + s + " | 시작: " + time.format(DateTimeUtils.DATETIME_FMT));
                } else {
                    System.out.println("시간표 없음");
                }
            } else if (c.equals("d")) {
                System.out.print("ID: ");
                ...
                String input = sc.nextLine();
                DayOfWeek d = DateTimeUtils.parseKoreanDow(input);
                List<Schedule> l = vm.byDay(d);
                for (int i = 0; i < l.size(); i++) {
    ...
    // author: hxeonsu, expected: 성적 메뉴 인터랙션 , developed: 과목검증/평균/GPA/요약/목록/수정/삭제 지원
    static void handleGrade(Scanner sc, GradeViewModel vm, ScheduleViewModel schedVM){
        while(true){
            System.out.println("========================================");
            ...
            System.out.println(" sum   : 요약");
            System.out.println(" course: 시간표 과목");
            System.out.println(" b     : 뒤로가기");
            System.out.println("----------------------------------------");
            System.out.print("> ");
            String c = sc.nextLine().trim();
            ...
            switch (c){
                case "a":
                    System.out.print("과목명(시간표 등록된 과목만): "); String cn = sc.nextLine();
                    System.out.print("점수(0~100): "); double scv = Double.parseDouble(sc.nextLine());
                    GradeEntry g = vm.add(cn, scv);
                    System.out.println("추가됨: " + g);
                    break;
                case "u":
                    System.out.print("ID: "); long id = Long.parseLong(sc.nextLine());
                    System.out.print("새 과목명(변경 없으면 Enter): "); String newC = sc.nextLine();
                    System.out.print("새 점수(변경 없으면 Enter): "); String ns = sc.nextLine();
                    vm.update(id, newC.isBlank()? null: newC, ns.isBlank()? null: Double.parseDouble(ns));
                    System.out.println("수정 완료");
                    break;
                case "d":
                    System.out.print("ID: "); long del = Long.parseLong(sc.nextLine());
                    vm.delete(del); System.out.println("삭제됨");
                    break;
                case "l":
                    List<GradeEntry> list = vm.list();
                    if (list.isEmpty()) System.out.println("(등록된 성적 없음)");
                    else list.forEach(System.out::println);
                    break;
                case "avg":
                    System.out.printf(Locale.ROOT, "평균 점수: %.1f%n", vm.averageScore());
                    break;
                case "gpa":
                    String letter = vm.gpaLetter();
                    double point = GradeViewModel.toGpaPoint(letter);
                    System.out.println("GPA: " + letter + " (" + point + " / 4.5)");
                    break;
                case "sum":
                    Map<String, Object> s = vm.summary();
                    System.out.println("[성적 요약]");
                    System.out.println(" 과목 수: " + s.get("count"));
                    System.out.println(" 평균: " + s.get("avg"));
                    System.out.println(" GPA: " + s.get("gpaLetter") + " (" + s.get("gpaPoint") + " / 4.5)");
                    System.out.println(" 최고점: " + s.get("max"));
                    System.out.println(" 최저점: " + s.get("min"));
                    break;
                case "course":
                    System.out.println("[시간표 등록 과목]");
                    Set<String> names = schedVM.allCourseNames();
                    if (names.isEmpty()) System.out.println(" (시간표가 비어 있습니다)");

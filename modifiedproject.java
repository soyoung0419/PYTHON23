...
public class CampusPlannerConsole {
    ...
    // ====== Utils ======
    // author: jaeeun0109, soyoung0419, hxeonsu, expected: 날짜·시간 파싱 유틸 제공 , developed: DateTimeFormatter 기반 파싱 및 한/영 요일 변환
    ...
    // ====== Models (Serializable) ======
    // author: jaeeun0109, expected: 할일(Task) 구조·마감·완료 상태 관리 , developed: id/제목/마감/완료/메모 + 임박 판정 + 문자열 표현
    static class Task implements Serializable {
        ...
        Task(long id, String title, LocalDateTime dueAt, boolean completed, String memo) {
            if (title == null) {
                this.title = "";
            } else {
                this.title = title;
        ...
        boolean isDueSoon(Duration threshold) {
            if (completed) {
                return false;
            } else {
                if (dueAt == null) {
                    ...
                    Duration nowToDue = Duration.between(LocalDateTime.now(), dueAt);
                    if (nowToDue.toMillis() <= threshold.toMillis()) {
                        return true;
                    } else {
                        return false;
                    }
        ...
        void update(String newTitle, LocalDateTime newDueAt,
                    String newMemo, String newCategory, int newPriority) {

            if (newTitle != null) this.title = newTitle;

            if (newDueAt != null) this.dueAt = newDueAt;

            if (newMemo != null) this.memo = newMemo;

            if (newCategory != null) this.category = newCategory;

            if (newPriority != -99999) {
    ...
    // author: jaeeun0109, expected: 주간 시간표 항목 및 겹침 판정 , developed: 요일/시간대 보유 + overlaps/nextOccurrenceFrom 구현
    static class Schedule implements Serializable {
        private static final long serialVersionUID = 1L;
        final long id;
        String courseName;
        String professor;
        ...
        Schedule(long id, String courseName, String professor, String room, DayOfWeek dow, LocalTime start, LocalTime end, String memo) {
            if (courseName == null) {
                this.courseName = "";
            } else {
        ...
        LocalDateTime nextOccurrenceFrom(LocalDateTime base) {
            int daysUntil = dow.getValue() - base.getDayOfWeek().getValue();
            while (daysUntil < 0) {
                daysUntil = daysUntil + 7;
            }
            LocalDate date = base.toLocalDate().plusDays(daysUntil);
            LocalDateTime candidate = LocalDateTime.of(date, start);
            if (candidate.isBefore(base)) {
                candidate = candidate.plusDays(7);
            }
            LocalDate d2 = candidate.toLocalDate();
            LocalTime t2 = candidate.toLocalTime();
            LocalDateTime result = LocalDateTime.of(d2, t2);
            return result;
        }
        ...
        public String toString() {
            ...
            if (start == null) {
                ...
                s5 = start.format(DateTimeUtils.TIME_FMT);
            }
            ...
            if (end == null) {
                ...
                s6 = end.format(DateTimeUtils.TIME_FMT);
            }
